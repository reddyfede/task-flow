...
end_workday_time = datetime.strptime(str(time(calendar[start_day_str][1][1], 0)), "%H:%M:%S")

start_time = datetime.strptime(str(time(start_datetime.hour, start_datetime.minute)), "%H:%M:%S")
remaining_time = (end_workday_time - start_time).total_seconds()/60
print("\nminutes to end of day:",remaining_time)


if remaining_time - 60*(calendar[start_day_str][1][1]-calendar[start_day_str][0][1]) > duration:
  print('\nenough time to end task in the morning')
  actual_end_time = end_datetime
  print('actual end time:', actual_end_time)
elif (start_hour >= calendar[start_day_str][0][0] and start_hour < calendar[start_day_str][0][1]) and (remaining_time > duration + 60):
  print('\ntasnk start in the morning and enough time to end task in the afternoon')
  actual_end_time = end_datetime + timedelta(minutes = 60)
  print('actual end time:', actual_end_time)
elif (start_hour >= calendar[start_day_str][1][0] and start_hour < calendar[start_day_str][1][1]) and (remaining_time > duration):
  ...
  if start_hour >= calendar[start_day_str][0][0] and start_hour < calendar[start_day_str][0][1]:
    allocated_today -= 60
  ...
  tomorrow_end_time = (datetime.strptime(str(time(calendar[conversion[tomorrow]][0][0], 0)), "%H:%M:%S") + timedelta(minutes=allocated_tomorrow)).time()
